...
public class TestFramework001 {
	...
	public void beforeMethod() throws Exception {
		...
		// For Constant Variables please see
		...
		// Now we do need to provide the Selenium driver to any of the Page
		// classes or Module Actions
	...
	public void main() throws Exception {
		...
		try {
			...
			// (iTestCaseRow)
			// This is called Modularization, when we club series of actions in
			// to one Module
			// For Modular Driven Framework, please see
			// http://www.toolsqa.com/modular-driven/
			SignIn_Action.Execute(iTestCaseRow);

			// This action is to select the Product category from the Top
			// Navigation of the Home Page
			// I have converted this in to a module, as there are so many logics
			// involved in to this selection
			// And it is always a best idea to keep your logics separate from
			// your test case
			ProductSelect_Action.productType(iTestCaseRow);

			// This action is to select the Product from the Product Listing
			// Page
			// I have again converted this in to a module, as there are so many
			// logics involved in to this selection
			ProductSelect_Action.productNumber(iTestCaseRow);

			// This is to assigning Product Name & Price to the variables from
			// the Check Out page, so that it can be matched later for
			// verification
			CheckOut_Action.Execute();

			// Here we are calling the Payment Details Action and passing
			// argument (iTestCaseRow)
			// This action will provide all the personal detail and payment
			// detail on the page and complete the payment for the selected
			// product
			PaymentDetails_Action.execute(iTestCaseRow);

			// This is to assigning Product Name & Price to the variables from
			// the Confirmation page, so that it can be matched later for
			// verification
			Confirmation_Action.Execute();

			// This is to match the Product Name & Price we have stored in
			// variables of Checkout & Confirmation page
			Verification_Action.Execute();

			// Now your test is about to finish but before that you need to take
			// decision to Pass your test or Fail
			// For selenium your test is pass, as you do not face any exception
			// and you come to the end or you test did not stop anywhere
